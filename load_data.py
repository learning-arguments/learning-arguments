...
def bin_data_set(data_set: pd.DataFrame, bin_labels: list) -> pd.DataFrame:
    ...
    for col in data_set.columns:
        bin_edges = np.linspace(np.min(data_set[col]) - 1e-5, np.max(data_set[col]) + 1e-5, len(bin_labels) + 1)
        column_labels = [label + '_' + col for label in bin_labels]
        binned_data[col + '_facts'] = pd.cut(data_set[col], bins=bin_edges, labels=column_labels)
    return binned_data
...
def generate_case_model(binned_data: pd.DataFrame) -> CaseModel:
    ...
    for i in binned_data.index:
        ...
        list_cases.append(Case(fact_set=fact_set, probability=1 / len(binned_data), name=case_name))
